...
pnpm install
```
4. Creare la cartella input, se non esiste
```bash
mkdir -p data/input
```
Windows PowerShell:
...
pnpm start
```
Anche in produzione l’app legge i file dalla cartella locale `data/input` rispetto alla root progetto, salvo override con `SIRCO_INPUT_DIR`.
...
La cartella host `./data/input` è montata nel container come `/app/data/input`.

## Variabili ambiente
Variabile opzionale:
- `SIRCO_INPUT_DIR`

Se valorizzata, l’app legge i file da questa cartella invece di `data/input`.
...
- Non aprire e salvare i file con Excel prima della validazione.
- L’app non modifica i file originali.
- Il report CSV errori è solo un report di controllo.
- La gestione anagrafica verrà aggiunta in una fase successiva.
