...
export function validateRelationsWithoutAnagrafica(tables: ParsedMap): ValidationIssue[] {
  ...
  for (const b of tables.B) {
    if (!tables.C.some((r) => r.relationKey === b.relationKey)) issues.push(parentIssue(b, "C", `Il Ricovero ${b.relationKey} deve avere almeno un record C`));
    if (!tables.D.some((r) => r.relationKey === b.relationKey)) issues.push(parentIssue(b, "D", `Il Ricovero ${b.relationKey} deve avere almeno un record D`));
  }
  for (const b of tables.B) {
